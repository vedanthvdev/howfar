...
function broadcastEnabled(enabled) {
  ...
  // Use the promise form of tabs.query — the callback form is on the MV3
  // deprecation path and has been observed to silently no-op in some 2024+
  // Chrome builds when invoked from a service worker. Wrap in a try/catch so
  // a transient API failure here can't take down the SET_ENABLED handler.
  (async () => {
    try {
